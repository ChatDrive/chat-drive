...
interface ColorAction {
  scene: ColorScene;
  handler: (colorValue: string) => string;
}

/** 获取主题颜色的各种场景对应的颜色 */
function getThemeColors(colors: [ColorType, string][]) {
  const colorActions: ColorAction[] = [
    { scene: '', handler: (colorValue) => colorValue },
    { scene: 'Suppl', handler: (colorValue) => colorValue },
    { scene: 'Hover', handler: (colorValue) => getColorPalette(colorValue, 5) },
    {
      scene: 'Pressed',
      handler: (colorValue) => getColorPalette(colorValue, 7),
    },
    {
      scene: 'Active',
      handler: (colorValue) => addColorAlpha(colorValue, 0.1),
    },
  ];
